...
export function AboutEnhanced() {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-white via-slate-50 to-blue-50 dark:from-gray-900 dark:to-blue-900 transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center space-y-4">
            <h2 className="heading-2 bg-gradient-to-r from-[#1254FF] to-[#00C4FF] bg-clip-text text-transparent">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <motion.div variants={itemVariants} className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="heading-3 text-primary">Key Expertise</h3>
                <div className="grid grid-cols-2 gap-3">
                  {ABOUT_SECTION_CONTENT.skills.map((skill, idx) => (
                    <motion.div
                      key={idx}
                      className="px-4 py-2 rounded-lg bg-blue-50 border border-blue-200 text-blue-700 text-sm font-medium dark:bg-[#00C4FF] dark:bg-opacity-20 dark:border-[#00C4FF] dark:border-opacity-30 dark:text-[#00C4FF]"
              ...
              <div className="space-y-4">
                <h3 className="heading-3 text-primary">Experience</h3>
                <div className="space-y-3">
                  {ABOUT_SECTION_CONTENT.experience.map((exp, idx) => (
                    <motion.div
                      key={idx}
                      className="p-4 rounded-lg bg-white/80 border border-slate-200 shadow-sm dark:bg-gray-800/50 dark:border-[#1254FF]/20"
                      whileHover={{ scale: 1.02 }}
                    >
                      <h4 className="font-semibold text-secondary">{exp.title}</h4>
                      <p className="text-sm text-muted">{exp.company} • {exp.period}</p>
                      <p className="text-sm text-slate-600 dark:text-gray-300 mt-1">{exp.description}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h3 className="heading-3 text-primary">Key Achievements</h3>
